...
public class UserValidate {
	...
	*/
	...
	/**
	 *  此方法用于判断输入的字符串是否包含字母和数字
	 * @param inputStr
	 * @return
	 */
	public static boolean isNumericAndLetter(String inputStr) {
		boolean isDigit = false;// 定义一个boolean值，用来表示是否包含数字
		boolean isLetter = false;// 定义一个boolean值，用来表示是否包含字母
		for (int i = 0; i < inputStr.length(); i++) {
			if (Character.isDigit(inputStr.charAt(i))) { // 用char包装类中的判断数字的方法判断每一个字符
				isDigit = true;
			}
			if (isAlpha(inputStr.charAt(i))) { // 用char包装类中的判断字母的方法判断每一个字符
				isLetter = true;
			}
		}
		if (isDigit && isLetter) {
	...
	public static void main(String[] args) {
		System.out.println(isNumericAndLetter("1234"));
	}
